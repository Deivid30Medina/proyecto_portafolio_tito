export function loadDataSliderPublicaciones() {
    return fetch('../../data/publicaciones/publicaciones-data.json')
        ...
        .then(data => {
            ...
            // Agregar la ruta relativa a las imágenes
            data.data = agregarRutaRelativa(data.data);
            console.log(data);
...
function agregarRutaRelativa(data) {
    return data.map(item => ({
        ...item,
        imagen_principal: `../.${item.imagen_principal}`,
        imagen_secundaria: `../.${item.imagen_secundaria}`,
        enlace: `../.${item.enlace}`
    }));
}
